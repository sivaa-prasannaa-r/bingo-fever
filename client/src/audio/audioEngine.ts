...
class AudioEngine {
  ...
  // ─── Primitive tone ──────────────────────────────────────────────────────────
  private _tone(freq: number, type: OscillatorType = 'sine', duration = 0.2, vol = 0.3, dest: AudioNode | null = null): void {
    if (!this.ctx) return;
  ...
  private _chord(freqs: number[], type: OscillatorType = 'sine', duration = 0.5, vol = 0.25): void {
  ...
  playNumberReveal(number: number): void {
  ...
  stopBGM(): void {
    if (this._bgmTimer !== null) clearInterval(this._bgmTimer);
    this._bgmTimer = null;
  }
}
